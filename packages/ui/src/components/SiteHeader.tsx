...
export function SiteHeader({
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-charcoal-200 bg-white/95 backdrop-blur">
      {/*
        Logo/CTAs stay on one always-visible row; nav links get their own
        row below and are allowed to wrap. A single-row layout with all
        three groups broke (overlapping text) on sites with long nav lists
        like tampabaylodgingco.com's 10 items — decoupling nav from the
        CTA row fixes that regardless of item count.
      */}
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <Link
            ...
            className="whitespace-nowrap text-xs font-medium text-charcoal-500 hover:text-brand-teal"
          >
            {crossSiteLink.label}
          </Link>
          {secondaryCta && (
            <ButtonLink href={secondaryCta.href} variant="secondary" size="sm" className="whitespace-nowrap">
              {secondaryCta.label}
            </ButtonLink>
          )}
          <ButtonLink href={primaryCta.href} variant="accent" size="sm" className="whitespace-nowrap">
            {primaryCta.label}
          </ButtonLink>
        </div>
      ...
      <nav
        className="hidden flex-wrap items-center gap-x-6 gap-y-2 border-t border-charcoal-100 px-4 py-2.5 sm:px-6 lg:flex"
        aria-label="Primary"
      >
        {navLinks.map((link) => (
          <Link
            key={link.href}
            href={link.href}
            className="whitespace-nowrap text-sm font-medium text-charcoal-700 hover:text-brand-teal"
          >
            {link.label}
          </Link>
        ))}
